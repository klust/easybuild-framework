...
class IntelIccIfortPSXE(Compiler):
    ...
    COMPILER_MODULE_NAME      = ['icc']
    compiler_environment_name = ['icc', 'ifort']
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(IntelIccIfortPSXE, self).__init__(*args, **kwargs)

    def _set_compiler_vars(self):
        """Intel compilers-specific adjustments after setting compiler variables."""
        super(IntelIccIfortPSXE, self)._set_compiler_vars()

        if not ('icc' in self.compiler_environment_name and 'ifort' in self.compiler_environment_name):
            raise EasyBuildError("_set_compiler_vars: missing icc and/or ifort EBVAR from COMPILER_MODULE_NAME %s",
                                 self.compiler_environment_name)

        # icc_root = self.get_software_root(self.COMPILER_MODULE_NAME[0])
        icc_root, _ = self.get_software_root(self.compiler_environment_name)[0:2]
        icc_version, ifort_version = self.get_software_version(self.compiler_environment_name)[0:2]
    ...
    def set_variables(self):
        ...
        icc_version, _ = self.get_software_version(self.compiler_environment_name)[0:2]
        if LooseVersion(icc_version) < LooseVersion('12'):
            self.options.options_map['openmp'] = 'openmp'

        super(IntelIccIfortPSXE, self).set_variables()
